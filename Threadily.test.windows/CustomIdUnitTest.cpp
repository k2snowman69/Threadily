		...
		public:
			...
			*/
			TEST_METHOD(GetOrCreateReturnsCustomId)
			{
				...
				Assert::AreEqual(id.businessId, product_UI->getId().businessId, L"UI Business Id not correct");
				Assert::AreEqual(id.productId, product_UI->getId().productId, L"UI Product Id not correct");
				...
				product_UI->runOnPeer(ThreadIds::ThreadId_Service, [&re, &id](std::shared_ptr<IThreadObject> peer) {
					...
					// verify that the ID was set properly
					Assert::AreEqual(id.businessId, product_Service->getId().businessId, L"Service Business Id not correct");
					Assert::AreEqual(id.productId, product_Service->getId().productId, L"Service Product Id not correct");

					// Now see if editing the old id will edit this guys Id
					id.businessId += 20;
					Assert::AreNotEqual(id.businessId, product_Service->getId().businessId, L"Service Business Id not correct");
					Assert::AreEqual(id.productId, product_Service->getId().productId, L"Service Product Id not correct");
